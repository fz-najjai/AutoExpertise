...
export default function AdminLayout({ children }) {
  ...
  const menuItems = [
    ...
    { name: 'Validation Experts', icon: ShieldCheck, path: '/admin/experts' },
    { name: 'Clients & Comptes', icon: Users, path: '/admin/users' },
  ];
      ...
      <aside className="hidden lg:flex w-72 flex-col fixed inset-y-0 z-50">
        <div className="flex-1 flex flex-col min-h-0 bg-[#020617] border-r border-indigo-900/40 relative overflow-hidden">
          ...
          <div className="flex-1 flex flex-col pt-8 pb-4 overflow-y-auto relative z-10">
            <div className="flex items-center flex-shrink-0 px-8 gap-3 mb-12 group cursor-pointer" onClick={() => navigate('/admin/dashboard')}>
            ...
            <nav className="flex-1 px-4 space-y-2">
              {menuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className={`mr-4 h-5 w-5 transition-colors ${isActive ? 'text-indigo-400' : 'text-indigo-800 group-hover:text-indigo-400'}`} />
                    {item.name}
                  </Link>
                );
              })}
      ...
      <div className="lg:pl-72 flex flex-col flex-1 min-h-screen">
        ...
        <header className="sticky top-0 z-40 bg-[#020617]/80 backdrop-blur-xl border-b border-indigo-900/40 px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <button className="relative p-2 text-indigo-400/50 hover:text-indigo-400 transition-colors group">
            ...
            <nav className="flex-1 p-6 space-y-2 overflow-y-auto">
              {menuItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className="flex items-center px-4 py-3 text-sm font-black text-indigo-200/50 rounded-2xl hover:bg-indigo-900/20 hover:text-indigo-400 transition-colors"
